...
import { pbxTiers, linePlans, addons, homeProducts, HomeStyle, Plan } from '@/lib/products';
...
type Family = 'home' | 'business' | 'pbx' | 'trunk';
...
// Every addon that could plausibly attach to something built here. IVR is
// offered on lines (home and business); PBX tiers already include it and it
// needs a line or PBX to attach to, so it never shows for PBX or trunk-only.
...
export default function PlanBuilder() {
  ...
  // #home-line etc. preselect that family. The ids themselves live on the
  // comparison tables, so the browser handles the scroll; this only syncs the
  // builder to the same family.
  useEffect(() => {
    const sync = () => {
  ...
  const eligibleAddons = addons.filter((addon) => {
    ...
    if (addon.id === 'addon-virtual-receptionist') return family === 'home' || family === 'business';
    return true;
  });
  ...
  const cartUrl = orderCartUrl([base, ...(trunkAddOn ? [trunkAddOn] : []), ...selectedAddons]);
  ...
  function changeFamily(next: Family) {
    setFamily(next);
    // Drop toggles for addons that stop being offered in the new family.
    setAddonIds((prev) => prev.filter((id) => id !== 'addon-virtual-receptionist' || next === 'home' || next === 'business'));
  }
          ...
          <div className="mt-6">
            ...
            <div className="mt-2 space-y-2">
              {eligibleAddons.map((addon) => {
                ...
                return (
                  <div key={addon.id} className="flex items-center justify-between rounded-xl border border-navy-900/10 px-4 py-2.5 text-sm">
